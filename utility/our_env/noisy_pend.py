...
class noisyPendulumEnv(gym.Env):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, render_mode: Optional[str] = None, dt = 0.05,g=10.0,sigma = 0.0,max_episode_steps = 200):
        self.max_speed = 8
        # self.max_torque = 2.0
        self.max_torque = 8. #try larger max_torque
        # self.dt = 0.05
        self.dt = dt #try smaller dt
        self.g = g
        self.m = 1.0
        self.l = 1.0
        self.sigma = sigma
    ...
    def step_koopman(self, u):
        ...
        if self.render_mode == "human":
            self.render()
        return self.normalize_state(self.state), -costs, done, {}
        # return self.state, -costs,done,{} #try returning unnormalized state
    ...
    def normalize_state(self, state):
        ...
        return np.asarray([angle_normalize(self.state[0]), self.state[1]])   

    def reset_koopman(self, *, init_state = None, seed: Optional[int] = None, options: Optional[dict] = None):
        super().reset(seed=seed)
        if options is None:
            high = np.array([DEFAULT_X, DEFAULT_Y])
        else:
        ...
        # print("init state shape", self.state.shape)
    ...
    def visualize(self, init_state, cmd, dt: float =None):
        ...
        if dt is None:
            dt = self.dt
        self.reset_koopman(init_state = init_state)
        for ctrl in cmd:
            self.render()
            time.sleep(dt)
            self.step(ctrl)
