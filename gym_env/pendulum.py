...
class PendulumEnv(gym.Env):
    ...
    def single_pendulum(self,y,t, u):
        theta, dtheta = y
        f = np.asarray([dtheta, 3.0*self.g/(2.0*self.l) * np.sin(theta) + 3.0/ (self.m * self.l ** 2)*u ])
        return f
    ...
    def step(self, u):
        ...
        # if newth <= -np.pi:
        #     newth += 2*np.pi
        # newstate = odeint(self.single_pendulum, self.state, [0, self.dt], args=(u,))
        # newth, newthdot = newstate[-1,:]
