...
def main(env,log_path,agent,rf_num, learn_rf, max_steps = 200, tr_seed = 0, true_state_dim = None, save_traj = False, euler = False, sigma = 0.0):
  ...
  print(f"mean episodic reward over {max_steps} time steps (rf_num = {rf_num}, learn_rf = {learn_rf}, seed ={tr_seed}, euler = {euler}, sigma = {sigma}): ", np.mean(all_rewards))
  if save_traj == True:
    filename = f"traj_log/traj_rf_num={rf_num}_learn_rf={learn_rf}_tr_seed={tr_seed}_eval_seed={eval_seed}_euler={euler}_sigma={sigma}.npy"
    os.makedirs(os.path.dirname(filename), exist_ok = True)
    with open(filename, 'wb') as f:
      np.save(f, all_traj)

  filename = f"cost_log/cost_rf_num={rf_num}_learn_rf={learn_rf}_tr_seed={tr_seed}_eval_seed={eval_seed}_euler={euler}_sigma={sigma}.npy"
  os.makedirs(os.path.dirname(filename), exist_ok = True)
  with open(filename, 'wb') as f:
