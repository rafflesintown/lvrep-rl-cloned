...
def main(env,log_path,agent,max_steps = 100,state_dim = None):
  np.random.seed(0)
  # n_init_states = 1000
  low_arr =  [-np.pi,-8.]
  high_arr = [np.pi, 8.]
  ...
  print("mean episodic reward over 100 time steps", np.mean(all_rewards))
...
if __name__ == "__main__":
  ...
  if args.env == "Pendulum-v1":
    ...
    main(env, log_path,agent, state_dim = 2)
